...
/*
	...
	Written by Bjornar Henden <bhenden@online.no>
     modified by sweetlilmre@ps2dev 2005/11/06
...
static HANDLE playThreadHandle = NULL;
static HANDLE exitEventHandle = NULL;
...
WORD WINAPI PlayThread(LPVOID pParam)
{
  while(WAIT_OBJECT_0 != WaitForSingleObject(exitEventHandle, 0))
  {
    WIN_Update();
  }
	return 0;
...
static BOOL WIN_Init(void)
{
  ...
  exitEventHandle = CreateEvent(NULL, TRUE, FALSE, "mm_WinPlayEvent");
	playThreadHandle = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) PlayThread, 0, 0, &threadID);
...
static void WIN_Update(void)
{
	...
	while (buffersout<NUMBUFFERS) 
  {
    ...
    {
      memset(buffer[nextbuffer], 0, buffersize);
    }
		if (!done) break;
...
static void WIN_PlayStart(void)
{
	VC_PlayStart();
  playing = 1;
}

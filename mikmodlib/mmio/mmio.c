...
FILE *_mm_fopen(CHAR *fname, CHAR *attrib)
{
    ...
    if (_mm_io_size)
    {
	    ...
	    fread(_mm_io_buffer, _mm_io_size, 1, fp);
    }
    else
    {
    		_mm_io_buffer = NULL;
...
int _mm_feof(FILE *fp)
{
    return _mm_eof;
}

int _mm_fread(void *buffer, size_t size, size_t count, FILE *fp)
{
